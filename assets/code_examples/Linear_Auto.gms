...
*External data
$INCLUDE "data\auto.dat";
*$INCLUDE "C:\Users\AGolan\Desktop\Tutotial_May_2016\LAB_4\auto.dat";
...
display   lambdasdual, pdual, betadual, v;
...
***************************************************************
*--------------------------------------------------------------
* Generalized Maximum Entropy - STOCHASTIC MOMENTS                             |
*--------------------------------------------------------------
***************************************************************
***
V(j) = V(j)/sqrt(card(t));
...
nm_con(k)..                sum((t,kk,m), x(t,kk)*x(t,k)*nm_p(kk,m)*z(m))
                         + ((sum((t), x(t,k)* (sum(j, nm_w(k,j)*v(j)))))) =e=
                           ((sum((t), x(t,k)*y(t))));
...
Solve GMEM maximizing obj using nlp;
...
display betaprimal, varB, SE_beta, sigma2, omega2;
